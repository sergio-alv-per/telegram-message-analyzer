...
class SeriesTiempo():
    ...
    def hora_y_minuto(self, dt_mensaje):
        hora = time(hour=dt_mensaje.time().hour,
                    minute=dt_mensaje.time().minute)
        return hora
    ...
    def añadir_mensaje(self, mensaje):
        ...
        self.series_tiempo[autor_mensaje]["mensajes_por_dia"][self.fecha(
            dt_mensaje)] += 1
        self.series_tiempo[autor_mensaje]["mensajes_por_hora"][self.hora_del_dia(
            dt_mensaje)] += 1
        self.series_tiempo[autor_mensaje]["mensajes_por_minuto"][self.hora_y_minuto(
            dt_mensaje)] += 1
        self.series_tiempo[autor_mensaje]["mensajes_por_dia_año"][self.dia_del_año(dt_mensaje)] += 1
        self.series_tiempo[autor_mensaje]["mensajes_por_dia_semana"][self.dia_de_semana(
            dt_mensaje)] += 1
