...
def exportar_datos_conversacion(datos_conversacion, nombre_directorio):
    exportar_recuentos_mensajes(datos_conversacion["recuentos_mensajes"], nombre_directorio)
    exportar_series_tiempo(datos_conversacion["series_tiempo"], nombre_directorio)
    exportar_frecuencia_palabras(datos_conversacion["frecuencia_palabras"], nombre_directorio)
...
def exportar_series_tiempo(series_tiempo, nombre_directorio):
    for emisor, series in series_tiempo.items():
        for nombre_serie, serie in series.items():
            archivo = os.path.join(nombre_directorio, f"series_tiempo_{nombre_serie}_{emisor}.csv")
            serie.to_csv(archivo, encoding="UTF-8", header=False)
...
def exportar_frecuencia_palabras(frecuencia_palabras, nombre_directorio):
    for emisor, frecuencias in frecuencia_palabras.items():
        archivo = os.path.join(nombre_directorio, f"frecuencia_palabras_{emisor}.csv")
        frecuencias.to_csv(archivo, encoding="UTF-8")
